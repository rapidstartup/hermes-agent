...
def run_doctor(args):
    ...
    if env_path.exists():
        ...
        # Check process env (post load_hermes_dotenv) so stale KEY=*** placeholders
        # are not counted as configured; fall back to UTF-8 file read on ImportError.
        try:
            ...
            has_key = _has_provider_env_config(content)
        if has_key:
